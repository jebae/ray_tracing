...
: test(UnitTest("Ray Grid Props", print_success))
{}

void TestRayGridProps::test_construct_case1(void)
{
	test.set_subject("ray grid props has to be constructed");
	Camera cam(
	...
	test.eq(ray_grid_props.e, cam.pos);

	ray_grid_props.offset.for_each(rounding);
	ray_grid_props.dx.for_each(rounding);
	ray_grid_props.dy.for_each(rounding);
	test.eq(ray_grid_props.offset, Vec4(vector<float>{-1.0f, 1.0f, 0.8f}));
	test.eq(ray_grid_props.dx, Vec4(vector<float>{0.002002f, 0, 0}));
	test.eq(ray_grid_props.dy, Vec4(vector<float>{0, 0, -0.002003}));
}
