...
class TestVec4 : public UnitTest
{
public:
...
: UnitTest("Vec4", print_success)
{}

void TestVec4::test_construct_case1(void)
{
	set_subject("default value has to be assigned for constructor");
	Vec4 v;
...
{
	set_subject("passed value has to be assigned for constructor");
...
{
	set_subject("[] operator has to work");
	Vec4 v;
...
{
	set_subject("== operator has to work with returning true");
	Vec4 v1;
	Vec4 v2;
	bool res = v1 == v2;
...
{
	set_subject("== operator has to work with returning false");
	Vec4 v1;
	Vec4 v2;
...
{
	set_subject("* operator has to scalar multiplication (s * v)");
	float scalar;
	Vec4 v1;
	Vec4 res;
	...
	eq(res[0], v1[0] * scalar);
	eq(res[1], v1[1] * scalar);
	eq(res[2], v1[2] * scalar);
	eq(res[3], 1.0f);
}

void TestVec4::test_scalar_mul_case2(void)
{
	set_subject("* operator has to scalar multiplication (v * s)");
	float scalar;
	Vec4 v1;
	Vec4 res;
...
{
	set_subject("*= operator has to scalar multiplication");
	float scalar;
	Vec4 v1;
	Vec4 res;
...
{
	set_subject("dot product has to work");
	Vec4 v1;
	Vec4 v2;
	float res;
	...
	res = v1.dot(v2);
	eq(res, v1[0] * v2[0] + v1[1] * v2[1] + v1[2] * v2[2]);
	res = v2.dot(v1);
	eq(res, v1[0] * v2[0] + v1[1] * v2[1] + v1[2] * v2[2]);
}
...
{
	set_subject("cross product has to work");
	Vec4 v1;
	Vec4 v2;
	Vec4 res;
	...
	res = v1.cross(v2);
	eq(res[0], v1[1] * v2[2] - v1[2] * v2[1]);
	eq(res[1], v1[2] * v2[0] - v1[0] * v2[2]);
	eq(res[2], v1[0] * v2[1] - v1[1] * v2[0]);
}

void TestVec4::test_norm_case1(void)
{
	set_subject("norm of vector has to be returned");
	Vec4 vec;
	float res;
	...
	vec[2] = 100.0f;
	res = vec.norm();
	eq(res, sqrtf(vec[0] * vec[0] + vec[1] * vec[1] + vec[2] * vec[2]));
}

void TestVec4::test_normalize_case1(void)
{
	set_subject("vector has to be normalized");
	Vec4 vec;
	float norm;
...
{
	set_subject("every element has to be affected by function");
	Vec4 vec;
